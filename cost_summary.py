...
def get_costs_by_service(client, account_id):
    today = datetime.date.today() # - relativedelta(months=1)  # Shift one month back
   ...
   # print(f"Debug: Date ranges used in get_cost_by_service - last_month_start: {last_month_start}, last_month_end: {last_month_end}, two_months_ago_start: {two_months_ago_start}, two_months_ago_end: {two_months_ago_end}")
...
def get_account_costs(client, accounts, account_names):
   ...
   # print(f"Debug: Date ranges used in get_account_costs - last_month_start: {last_month_start}, last_month_end: {last_month_end}, two_months_ago_start: {two_months_ago_start}, two_months_ago_end: {two_months_ago_end}")


    costs_last_month = get_cost(client, accounts, last_month_start, last_month_end)
    costs_two_months_ago = get_cost(client, accounts, two_months_ago_start, two_months_ago_end)
    costs_three_months_ago = get_cost(client, accounts, three_months_ago_start, three_months_ago_end)
    costs_four_months_ago = get_cost(client, accounts, four_months_ago_start, four_months_ago_end)
    costs_five_months_ago = get_cost(client, accounts, five_months_ago_start, five_months_ago_end)
...
# def colored_arrow(cost1, cost2):
#    arrow = "↑" if cost1 > cost2 else "↓"
#    color = "\033[91m" if arrow == "↑" else "\033[92m"
#    return f"{color}{arrow}\033[0m"
...
def display_cost_changes(client, accounts, account_costs, last_month_start, last_month_end, two_months_ago_start, two_months_ago_end, three_months_ago_start, four_months_ago_start, five_months_ago_start):
    ...
    for account_id, costs in sorted_account_costs:
        ...
        if two_months_ago_cost != 0:
            acc_pct = (last_month_cost - two_months_ago_cost) / two_months_ago_cost * 100
        else:
            acc_pct = 0  # or any other value that makes sense in your context
        ...
        if total_two_months_ago_cost != 0:
            total_pct = (total_last_month_cost - total_two_months_ago_cost) / total_two_months_ago_cost * 100
        else:
            total_pct = 0 
        ...
        # Create an array with the last 4 months' costs.
        costs_array = np.array([five_months_ago_cost, four_months_ago_cost, three_months_ago_cost, two_months_ago_cost, last_month_cost])

        # Calculate the slope of the best-fit line using the least squares method.
        slope = np.polyfit(range(len(costs_array)), costs_array, 1)[0]

        # Check if the slope is positive and if the total increase in costs is greater than a certain threshold.
        ...
        if zero_cost_months <= len(costs_array) // 2:
            if slope > 0 and long_term_increase_amount > 100:
                longer_term_increase_accounts.append((account_id, account_name, long_term_increase_amount, slope))
    ...
    print(tabulate([footer_row], headers=headers, tablefmt="pretty", numalign="right", stralign="right"))



    if significant_increase_accounts:
        print("\nTop 2 service cost increases last month:")
        significant_increase_accounts_sorted = sorted(significant_increase_accounts, key=lambda x: x[2], reverse=True)
        for account_id, account_name, increase_amount, increase_percentage in significant_increase_accounts_sorted:
    ...
    if longer_term_increase_accounts:
        ...
        longer_term_increase_accounts_sorted = sorted(longer_term_increase_accounts, key=lambda x: x[2], reverse=True)
        for account_id, account_name, increase_amount, increase_percentage in longer_term_increase_accounts_sorted:
            print(f"{account_id:>12}  {account_name:<20} {increase_amount:>10,.2f} {increase_percentage:>8,.2f}%")
